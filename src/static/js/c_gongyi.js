...
export class partLabsDescriptions extends partDescriptions {
    ...
    render(result)
    {
        ...
        result.forEach((dataItem, index) => {
            ...
            dataItem.order_id = this.data_params.product_id;           
            this.table_id = `${T_ENDPOINTS[this.title]||''}-${dataItem.material_model||''}`;
            this.select_params =  {
                optionText: '工艺', defaultText: '请选择历史工艺线路设计', key: 'id', textK: 'text',
                url: JSON.stringify(API_CONFIG[APP].route_selecter), endpoint: dataItem?.material_id || ''
            }
            this.button_group =  [{id:'row-remove', text:'撤销工序', className:'el-button el-button--warning el-button--small'},
