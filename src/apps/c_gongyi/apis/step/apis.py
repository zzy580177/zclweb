...
def create(request, payload: list[StepIn]):
    ...
    try:          
        ...
        existing_types = {attr.name: attr for attr in Attribute.objects.filter(description='设备')}
    ...
    for idx, item in enumerate(payload):
        try:
            ...
            if item.type_name not in existing_types:
                faileds.append({'id': idx, 'status': False, 'error': f"设备[{item.type_name}]不存在"})
                continue
            ...
            if any(name == item.name for _, _ , name in to_create):
                continue
